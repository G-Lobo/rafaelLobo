...
<div class="container mx-auto px-5 xl:px-32 xl:pr-16 pt-4 pb-16">
    ...
    <div class="flex flex-start place-content-center xl:place-content-start">
        <form method="GET" action="{{ route('movies.index') }}" class="flex items-center mx-0 mb-6">
            <label for="area_id" class="text-base font-bold text-black text-center pr-3">Filtrar por Área:</label>
    ...
    {{-- quadrinhos --}}
    <h2 class="text-4xl font-black text-center xl:text-left mx-auto mb-2 pt-8 pb-8">QUADRINHOS</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <a href="{{ route('comic.show', $comic->id)}}">
            <div class="flex bg-transparent overflow-hidden transform transition duration-300 hover:scale-105">
                <!-- comic Cover Art -->
                <img src="{{ asset('assets/img/comicCovers/' . $comic->coverImg) }}" alt="{{ $comic->title }}" class="w-1/3 rounded-lg object-scale-down h-64 xl:h-auto" />
                ...
                <div class="w-2/3 pl-5 xl:p-4 flex flex-col justify-start">
                    ...
                    <div class="flex flex-wrap space-x-2 mt-2">
                        <div>
                            <span class="bg-yellow-100 text-yellow-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{{ \Carbon\Carbon::parse($comic->releaseDate)->format('Y') }}</span>
                            {{-- <span class="bg-transparent text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{{ $comic->duration }} min</span> --}}
                        </div>
                    </div>
