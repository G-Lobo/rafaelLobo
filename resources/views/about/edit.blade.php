...
<div class="max-w-4xl mx-auto p-8 bg-gray-100 shadow-lg rounded-xl mt-8">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Editar Bio</h2>
    ...
    <form action="{{ route('about.update', [$bio->id]) }}" method="POST" class="space-y-6">
        @csrf
        @method('DELETE')
    ...
    <form action="/about/{{ $bio->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="content" id="quill-editor-area" class="hidden">{{ old('content', $bio->content) }}</textarea>
            <script>
        ...
        <div>
            ...
            <img src="/assets/img/profPic/{{ $bio->image }}" alt="" class="w-full max-w-xs rounded-lg mb-3">
